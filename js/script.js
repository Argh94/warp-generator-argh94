...
function toggleTheme() {
    ...
    if (isDarkTheme) {
        ...
        document.documentElement.style.setProperty('--container-bg', '#262626');
        document.documentElement.style.setProperty('--text-color', '#c9d1d9');
        document.documentElement.style.setProperty('--info-text-color', '#c9d1d9');
        document.documentElement.style.setProperty('--input-bg', '#333333');
        document.documentElement.style.setProperty('--input-border', '#30363d');
        document.documentElement.style.setProperty('--info-box-bg', '#3a3a3a');
    } else {
        ...
        document.documentElement.style.setProperty('--info-text-color', '#1f2937');
        document.documentElement.style.setProperty('--input-bg', '#f9fafb');
        document.documentElement.style.setProperty('--input-border', '#d1d5db');
        document.documentElement.style.setProperty('--info-box-bg', '#ffffff');
    }
...
function updateTexts() {
    const texts = {
        'fa': {
            ...
            'generateButton': 'ساخت کانفیگ',
            'loadingText': 'در حال تولید URL...',
            'generatedUrlTitle': 'URL تولید شده:',
            'copyUrlButton': 'کپی URL',
            'contactTitle': 'ارتباط با من',
            'footerDesigner': 'طراحی شده توسط: <a href="https://github.com/Argh94" target="_blank">Argh94</a>',
            'noisePacketsLabel': 'تعداد بسته‌های نویز',
            'noisePacketSizeLabel': 'اندازه بسته‌های نویز',
            'noisePacketDelayLabel': 'تأخیر ارسال بسته‌های نویز',
            'noiseModeLabel': 'حالت تولید نویز',
        ...
        'en': {
            ...
            'footerDesigner': 'Designed by: <a href="https://github.com/Argh94" target="_blank">Argh94</a>',
            'noisePacketsLabel': 'Number of noise packets',
            'noisePacketSizeLabel': 'Size of noise packets',
            'noisePacketDelayLabel': 'Delay of sending noise packets',
            'noiseModeLabel': 'Noise generation mode',
    ...
    document.querySelector('.copy-btn').innerHTML = `<i class="material-icons">content_copy</i> ${texts[currentLang]['copyLinkButton']}`;
    ...
    document.querySelector('.footer-info p').innerHTML = texts[currentLang]['footerDesigner'];
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('warpinatorForm').addEventListener('submit', function(event) {
        ...
        setTimeout(function() {
            ...
            var url;
            if (type === 'hiddify' || type === 'karing') {
                var username = document.getElementById('username').value || 'Argh94'; // پیش‌فرض Argh94
                var license = document.getElementById('license').value || '';
                var type1 = document.getElementById('type1').value || '50-100';
                var type2 = document.getElementById('type2').value || '50-100';
                var type3 = document.getElementById('type3').value || '3-6';
                ...
                if (warpInWarp) {
                    if (type === 'karing') {
                        var wowUsername;
                        if (customUsername) {
                            wowUsername = `${username}-WoW-${ipVersion === 'ipv6' ? 'IPv6' : 'IPv4'}`;
                        } else {
                            wowUsername = `Argh94-WoW-${ipVersion === 'ipv6' ? 'IPv6' : 'IPv4'}`;
                        }
                        if (ipVersion === 'ipv6') {
...
function copySubscriptionLink() {
    ...
    const copiedMessage = isEnglish ? 'Copied!' : 'کپی شد!';
    copyBtn.innerHTML = `<i class="material-icons">check</i> ${copiedMessage}`;
    setTimeout(function() {
        updateTexts();
    }, 2000);
}
